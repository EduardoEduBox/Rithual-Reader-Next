...
const Modal = ({openModal, setOpenModal}: any) => {
  ...
  return (
    <Transition.Root show={openModal} as={Fragment}>
      <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRef} onClose={() => setOpenModal(true)}>
        <Transition.Child
          as={Fragment}
...
const Dropdown = ({ setOpenModal }: { setOpenModal: (value: boolean) => void }) => {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <Menu.Button>
        <EllipsisHorizontalIcon className="mr-2 relative top-[10px] h-8 w-8 text-gray-400" aria-hidden="false" />
      </Menu.Button>
...
const Comments: React.FC<idProp> = async ({ id }) => {
  ...
  const updateLikes = async (commentIndex: number, comment: any, like: boolean) => {
